<template>
    <div class="containerr">
      <div class="table-container  shadow-md container p-4">
        ...
        <table class="w-full table-auto">
          <thead>
            <tr>
              <th class="px-4 py-2">Bordereau N°</th>
              <th class="px-4 py-2">N° Affaire</th>
              <th class="px-4 py-2">Réf. Client</th>
              <th class="px-4 py-2">Raison Sociale</th>
              <th class="px-4 py-2">N° BC Client</th>
              <th class="px-4 py-2">Date d'envoi</th>
              <th class="px-4 py-2">Date Reçu</th>
          ...
          <tbody>
            <tr v-for="production in displayedProductions" :key="production.id">
              <td class="px-4 py-2">{{ production.bordereauNumber }}</td>
              <td class="px-4 py-2">{{ production.affairNumber }}</td>
              <td class="px-4 py-2">{{ production.refClient }}</td>
              <td class="px-4 py-2">{{ production.raisonSociale }}</td>
              <td class="px-4 py-2">{{ production.bcClientNumber }}</td>
              <td class="px-4 py-2">{{ production.dateEnvoi }}</td>
              <td class="px-4 py-2">{{ production.dateReçu }}</td>
              <td class="px-4 py-2">{{ production.telephone }}</td>
              <td class="px-4 py-2">{{ production.date }}</td>
              <td class="px-4 flex gap-2 py-2">
                <a href="#">
                  <button title="valider" class="bg-blue-500 text-white px-2 py-1 rounded focus:outline-none focus:shadow-outline">
    ...
    data() {
      return {
        ...
        productions: [
          {
            id: 7,
            bordereauNumber: "BORD001",
            affairNumber: "AFF001",
            refClient: "REF001",
            raisonSociale: "Production 1",
            bcClientNumber: "BC001",
            dateEnvoi: "2024-01-24",
            dateReçu: "2024-01-25",
            telephone: "123456789",
            date: "2024-01-24",
          },
